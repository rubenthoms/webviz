...
import type { HoveredSeriesInfo, TimeseriesDisplayConfig } from "@modules/_shared/eCharts";
import {
...
export function TimeseriesTimeRecipe({
    ...
    const { groups, overlays, timestamps } = React.useMemo(() => {
        const g = generateTimeseriesGroups(
            dataConfig.numSubplots,
            dataConfig.numGroups,
            dataConfig.numMembers,
            dataConfig.numTimesteps,
    ...
    const formatSeriesLabel = React.useCallback((info: HoveredSeriesInfo): string => {
        switch (info.kind) {
            case "member":
                return `${MEMBER_LABEL} ${info.memberId}`;
            case "statistic":
    ...
    const onChartEvents = React.useMemo(() => ({ ...seriesEvents }), [seriesEvents]);
