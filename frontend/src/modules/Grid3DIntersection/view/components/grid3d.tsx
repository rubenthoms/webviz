...
export function Grid3D(props: Grid3DProps): JSX.Element {
    ...
    const [userPolyline, setUserPolyline] = React.useState<number[][]>([]);
    ...
    const userPolylinePointLayer = new PointsLayer({
        id: "user-polyline-point-layer",
        pointsData: userPolyline.flat(),
        color: [0, 255, 0, 255],
        pointRadius: 10,
        radiusUnits: "pixels",
        ZIncreasingDownwards: false,
    ...
    const userPolylineLineLayer = new GeoJsonLayer({
    ...
    const handleMouseEvent = React.useCallback(
            ...
            if (event.type === "click") {
                ...
                if (event.x && event.y) {
                    for (const info of event.infos) {
                        if ("layer" in info && info.layer?.id === "grid-3d-layer") {
                            if ("properties" in info) {
                                const properties = info.properties as Record<string, string>[];
                                for (const property of properties) {
                                    if (property.name && property.name.startsWith("Depth") && property.value) {
                                        depth = parseFloat(property.value);
                                        break;
                                    }
                                }
                                if (depth !== null) {
                    ...
                    if (depth !== null) {
                        const point = [event.x, event.y, depth];
                        setUserPolyline((prev) => [...prev, point]);
                    }
                }
            }
            if (event.type !== "hover" || !props.fieldWellboreTrajectoriesData) {
                return;
            }
                            ...
                            for (const property of properties) {
                                if (property.name && property.name.startsWith("MD") && property.value) {
                                    ...
                                    if ("coordinate" in info && info.coordinate) {
                                        coordinate = [info.coordinate[0], info.coordinate[1], -info.coordinate[2]];
                                        break;
                                    }
                                }
                            }
        ...
        [onHoveredMdChange, props.selectedWellboreUuid, setHoveredMdPoint]
    );
...
function SubsurfaceViewerWithCameraState(props: SubsurfaceViewerProps): React.ReactNode {
    const [cameraPosition, setCameraPosition] = React.useState<ViewStateType | undefined>(undefined);

    const handleCameraChange = React.useCallback(function handleCameraChange(viewport: ViewStateType): void {
