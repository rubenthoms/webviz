...
export function buildMemberSeries(trace: TimeseriesTrace, axisIndex = 0): SeriesBuildResult {
    ...
    const series: LineSeriesOption[] = trace.memberValues.map(function buildMemberLineSeries(memberValues, index) {
        ...
        console.debug(
            "Building member series for trace",
            trace.name,
            "member index",
            index,
            "member id",
...
export function buildMemberSeriesTime(trace: TimeseriesTrace, axisIndex = 0): SeriesBuildResult {
    if (!trace.memberValues) return { series: [], legendData: [] };
    ...
    const series: LineSeriesOption[] = trace.memberValues.map(function buildMemberLineSeriesTime(memberValues, index) {
        const memberId = trace.memberIds?.[index] ?? index;
        const color = trace.memberColors?.[index] ?? trace.color;
        ...
        return {
            ...
            name: trace.name,
            type: "line",
            data: timestamps.map((ts, i) => [ts, memberValues[i] ?? null]),
            xAxisIndex: axisIndex,
            yAxisIndex: axisIndex,
            itemStyle: { color },
            lineStyle: { color, width: 0.8, opacity: 0.4 },
