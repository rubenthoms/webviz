    ...
    for (const trace of group.traces) {
        if (config.showMembers && trace.memberValues) {
            const memberResult = useLargeMemberSeries
                ? buildMemberSeriesLarge(trace, axisIndex)
                : buildMemberSeries(trace, axisIndex);
            appendSeriesBuildResult(series, legendData, seenLegend, memberResult);

            // In large mode the custom series is a single opaque series object —
            // no per-member interaction entries are registered.
...
function appendSeriesBuildResult(
    targetSeries: ChartSeriesOption[],
    legendData: string[],
...
function appendMemberInteractionEntries(
    ...
    trace.memberValues?.forEach(function appendMemberInteractionEntry(memberValues, memberOffset) {
        const memberId = String(trace.memberIds?.[memberOffset] ?? memberOffset);
        const color = trace.memberColors?.[memberOffset] ?? trace.color;
